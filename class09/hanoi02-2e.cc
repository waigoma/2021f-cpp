...
void move(const string& from, const string& to) {
    if (from == "A") {
        if (plate_num[0] == 0) {
            throw invalid_argument("Error: no plate in pole A");
        }
        if (to == "B") {
            if (plate_num[1] == DISK_SIZE) {
                throw invalid_argument("Error: no space in pole B");
            }

            int bp = plate_num[1] == 0 ? 0 : board[1][plate_num[1] - 1];
            if (board[0][plate_num[0] - 1] > bp && bp != 0) {
                throw invalid_argument("Error: can't move A to B");
            }

            movePlate("A", "B");
        } else if (to == "C") {
            if (plate_num[2] == DISK_SIZE) {
                throw invalid_argument("Error: no space in pole C");
            }

            int cp = plate_num[2] == 0 ? 0 : board[2][plate_num[2] - 1];
            if (board[0][plate_num[0] - 1] > cp && cp != 0) {
                throw invalid_argument("Error: can't move A to C");
            }

            movePlate("A", "C");
        } else if (to == "A") {
            throw invalid_argument("Error: can't move to itself");
        }
    } else if (from == "B") {
        if (plate_num[1] == 0) {
            throw invalid_argument("Error: no plate in pole B");
        }
        if (to == "A") {
            if (plate_num[0] == DISK_SIZE) {
                throw invalid_argument("Error: no space in pole A");
            }

            int ap = plate_num[0] == 0 ? 0 : board[0][plate_num[0] - 1];
            if (board[1][plate_num[1] - 1] > ap && ap != 0) {
                throw invalid_argument("Error: can't move B to A");
            }
            ...
            if (board[1][plate_num[1] - 1] > cp && cp != 0) {
                throw invalid_argument("Error: can't move B to C");
            }
        ...
        if (plate_num[2] == 0) {
            throw invalid_argument("Error: no plate in pole C");
        }
        if (to == "A") {
            if (plate_num[0] == DISK_SIZE) {
                throw invalid_argument("Error: no space in pole A");
            ...
            if (board[2][plate_num[2] - 1] > ap && ap != 0) {
                throw invalid_argument("Error: can't move C to A");
            }
            ...
            if (board[2][plate_num[2] - 1] > bp && bp != 0) {
                throw invalid_argument("Error: can't move C to B");
            }
